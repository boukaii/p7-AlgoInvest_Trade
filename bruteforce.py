def bruteforce(depense_max, donnees, lst_actions_selectionees=[]):
    if donnees:
        """
        les variables val1, lst_val1 représentent le résultat de la fonction (rentabilité max, liste action) sans l'action courante
        """
        ...
        if action_selection[1] <= depense_max:
            """
            on enleve le montant de l'action en cours de la dépense max et on ajoute cette action dans la liste des actions
            """
